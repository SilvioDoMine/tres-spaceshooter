...
// Halos (um por coração, índices 0..MAX_HEARTS-1) e faíscas (resto) num único Points aditivo
const total = MAX_HEARTS + SPARKS;
...
for (let i = 0; i < MAX_HEARTS; i++) tints.set([haloColor.r, haloColor.g, haloColor.b], i * 3);
const sparkColors = [new Color('#ff3b5c'), new Color('#ffc2cf')];
const sparks = Array.from({ length: SPARKS }, (_, i) => ({ index: MAX_HEARTS + i, age: 1, life: 0, x: 0, y: 0, z: 0, vx: 0, vy: 0, vz: 0, size: 0 }));
let sparkCursor = 0, tintDirty = true;

function spark(x: number, y: number, z: number) {
  const s = sparks[sparkCursor++ % SPARKS]!;
  const angle = Math.random() * Math.PI * 2, offset = .15 + Math.random() * .15, speed = .5 + Math.random() * .7;
  Object.assign(s, {
    age: 0, life: .5 + Math.random() * .45,
    x: x + Math.cos(angle) * offset, y: y + .1, z: z + Math.sin(angle) * offset,
    vx: Math.cos(angle) * speed, vy: .8 + Math.random() * .6, vz: Math.sin(angle) * speed,
...
useLoop().onBeforeRender(({ delta }) => {
  ...
  for (let i = 0; i < MAX_HEARTS; i++) {
    ...
    const y = .45 + Math.sin(time * 2 + heart.id) * .08;
    dummy.position.set(heart.x, y, heart.z);
    dummy.rotation.set(0, 0, Math.sin(time * 1.8 + heart.id) * .6);
    dummy.scale.setScalar(.4 * (1 + beat * .2));
    dummy.updateMatrix();
    mesh.setMatrixAt(i, dummy.matrix);

    positions.set([heart.x, y - .3, heart.z], i * 3);
    radii[i] = 1.6 * (1 + beat * .3);
    opacities[i] = .5 + beat * .4;

    if (Math.random() < dt * SPARK_RATE) spark(heart.x, y, heart.z);
  }
