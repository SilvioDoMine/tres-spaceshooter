<script setup>
const names={1:'Órbita',2:'Estaleiro na nebulosa',3:'Comando da frota',4:'Maré de Esmeralda',5:'Jardim do Eclipse'}
const chapter=ref(4),freeCamera=ref(false)
 ...
 <main class="scenery-preview">
  ...
  <header><NuxtLink to="/">← Lobby</NuxtLink><h1>Capítulo {{chapter}} · {{names[chapter]}}</h1>
   ...
   <nav aria-label="Selecionar cenário">
    <button v-for="id in [1,2,3,4,5]" :key="id" :aria-pressed="chapter===id" @click="chapter=id" :title="names[id]">{{id}}</button>
   </nav>
      <details><summary>Controles da prévia</summary><nav aria-label="Câmera da prévia"><button @click="freeCamera=!freeCamera">{{freeCamera?'Usar câmera da partida':'Inspecionar em 3D'}}</button><button @click="x=0;z=0">Voltar ao spawn</button></nav>
